...
*/		
		...
		for (int i = 0; i< bmChapter.Length; i++) {
			...
			select_chapter[i] = EditorGUILayout.Toggle( bmChapter[i], select_chapter[i] );
		...
		for (int i = 0; i< bmHard.Length; i++) {
			...
			select_hard[i] = EditorGUILayout.Toggle( bmHard[i], select_hard[i]);
		...
		for (int i = 0; i< bmAttr.Length; i++) {
			...
			select_attr[i] = EditorGUILayout.Toggle( bmAttr[i], select_attr[i]);

		}
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		GUILayout.Label("小兵種類設定", GUILayout.Width(30f));
		this.Target.use_soldier_num = EditorGUILayout.IntField(this.Target.use_soldier_num, GUILayout.MaxWidth(50f));
		if (GUILayout.Button("產生小兵", GUILayout.Height(18f))){
		}
		GUILayout.EndHorizontal();
